...
function requireDesktopRelay(context?: ToolContext) {
  if (!context?.desktopRelay) {
    throw new Error('External app actions require the Peppa desktop client relay.');
  }
...
function buildDxf(args: Record<string, any>): string {
  ...
  if (args.titleBlock !== false) {
    const title = String(args.title || 'Peppa CAD Draft');
    const unit = String(args.unit || 'unit');
    const note = String(args.precisionNote || args.note || 'Draft generated by Peppa. Verify site dimensions before production.').slice(0, 120);
...
export function registerExternalAppTools(registry: ToolRegistry): void {
  registry.register({
    ...
    description: 'List Peppa external app adapters and their safety policies for browser, messaging, CAD, and other AI apps.',
    parameters: {
      type: 'object',
      properties: {},
      required: [],
  ...
  registry.register({
    ...
    handler: async (args, context) => {
      ...
      return JSON.stringify({
        ...
        note: 'The draft is ready on the clipboard. Peppa did not send the message.',
      }, null, 2);
    },
    permission: 'user',
    securityLevel: 'confirm',
  ...
  registry.register({
    ...
    parameters: {
      ...
      properties: {
        ...
        outputPath: { type: 'string', description: 'Optional exact DXF output path. Relative paths are resolved under outputDirectory or Peppa CAD data directory.' },
    ...
    handler: async (args, context) => {
      const title = safeFileName(String(args.title || 'peppa_cad_draft'));
