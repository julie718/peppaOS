...
const LEGAL_TRIAD_BASELINE = [
  '底层法律处理逻辑（不作为独立 UI 功能）：',
  '1. 大前提：先检索现行有效法律，解释法律构成要件，再用类案补强裁判规则。',
  '2. 小前提：把待证事实、证据材料、举证责任、质证风险逐项对应。',
  '3. 结论：完成涵摄判断，再转化为起诉状、答辩状、代理词、法律意见书、合同审查意见等工作产物。',
  '4. 所有未核验法条、未确认类案、未绑定证据的事实必须标注“待检索/待核验/待补证”。',
].join('\n');
...
## 底层三段论处理
- 大前提：合同审查先核验现行有效法律、示范文本和相关裁判规则。
- 小前提：将合同条款、履行背景、交易目的和证据材料对应到具体风险点。
- 结论：把涵摄结果转化为风险等级、修改建议和需律师确认的条款。
...
## 底层三段论处理
- 大前提：先核验现行有效法律、示范文本和行业规则。
- 小前提：将交易主体、标的、价款、履行、违约、争议解决等事实需求对应到条款。
- 结论：把涵摄结果转化为可填写、可复核、可谈判的合同条款。
...
## 底层三段论处理
- 大前提：围绕案由检索现行有效法律、司法解释和可比类案。
- 小前提：把案件事实拆成待证事实、已有关联证据、待补证据和对方可能质证点。
- 结论：把涵摄结果落到诉讼请求、抗辩方向、证据补强和风险控制。
...
## 三、底层三段论处理
- 大前提：围绕案由检索现行有效法律、司法解释和类案规则，未核验前不得写死条文。
- 小前提：将每个事实绑定证据页码、来源和证明目的；无法绑定的事实标注“待补证”。
- 结论：把涵摄结果转化为起诉状、答辩状、质证意见、代理词或法律意见书中的可复核表达。
...
## 三、三段论检索框架
- 大前提：先核验现行有效法律，再按最高院、高院、中院、基层法院顺序找类案规则。
- 小前提：围绕争议焦点拆分待证事实、证据材料、举证责任和质证风险。
- 结论：只把网页内人工确认后的来源登记为可引用素材，再进入文书包、代理词或法律意见书。
...
export function registerLegalTools(registry: ToolRegistry): void {
  ...
  registry.register({
    ...
    description: '内部法律工作底稿 — 按大前提（法律/类案）、小前提（事实/证据/举证质证）、结论（涵摄到文书表达）生成律师复核版分析。三段论是法律工具的底层处理逻辑，不作为前端独立功能页。',
    parameters: {
      type: 'object',
      properties: {
