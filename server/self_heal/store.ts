...
function open(): sqlite3.Database {
  ...
  // serialized 串行模式（OPEN_FULLMUTEX）+ 带回调构造：打开失败时错误进回调而非未捕获 'error' 事件
    const conn = new sqlite3.Database(getSelfHealDbPath(), OPEN_FLAGS, (err) => {
      if (err) {
...
async function execWithRetry<T>(fn: (conn: sqlite3.Database) => Promise<T>, opName: string): Promise<T> {
  ...
  for (let attempt = 0; attempt < BUSY_MAX_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      console.error(`[SelfHealDB] ${opName} 执行失败（已充分重试）:`, (err as any)?.message ?? err);
      throw err;
    }
  }
  console.error(`[SelfHealDB] ${opName} 执行失败（${BUSY_MAX_ATTEMPTS} 次尝试均未成功）:`, (lastErr as any)?.message ?? lastErr);
  throw lastErr;
}
...
export function closeSelfHealDb(): void {
  ...
  const conn = db;
  db = null;
  dbOpen = false; // 句柄失效标记：后续 open/语句自动重开连接，避免 SQLITE_MISUSE
  enqueueOp(() => new Promise<void>((resolve) => {
    conn.close((err) => {
      if (err) console.error('[SelfHealDB] 关闭失败:', err.message);
      resolve();
    });
  })).catch(() => {});
}
