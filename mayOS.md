...
| 2026-07-11 | GPS 排查未解决：多次修改后权限弹窗仍不出现，加手动测试按钮待验证 |
| ⬜ 待办 | 定时备份：cron 每天自动备份 ~/mayos/data/peppa.db 到 NAS 另一存储位置 |
| ⬜ GPS | 待验证：手动按钮触发 GPS，确认问题在插件层还是代码层 |
...
| iOS 设置→隐私→定位服务中有 MayOS | ❌ 没有 → 证明插件没调原生 API |

**当前状态：** 代码层面全部正确。问题锁定在——Capacitor Geolocation 插件的原生 `CLLocationManager.requestWhenInUseAuthorization()` 没有被调用。可能原因：useEffect 自动触发在 Capacitor WebView 中不满足 iOS 权限弹窗的用户手势要求。

**下一步：** 已加手动 GPS 测试按钮（黄色 `📍测试GPS`），用户点击触发，绕过 useEffect 限制。待推送到 NAS 验证。