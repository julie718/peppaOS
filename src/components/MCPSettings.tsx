      ...
      <div className="p-6 glass-dark rounded-[2rem] border border-white/5 space-y-4">
        ...
        <div className="flex gap-3">
          ...
          <Button
            ...
            className="bg-white/5 hover:bg-white/10 border border-white/10 text-[10px] font-black uppercase tracking-widest px-4 h-10 rounded-xl"
          >
            {ghSearching ? (t?.mcpSearching || 'Searching...') : (t?.mcpSearchBtn || 'Search')}
          </Button>
        </div>
        {ghSearched && ghResults.length === 0 && !ghSearching && (
          <p className="text-xs text-white/30">{t?.mcpNoResultsFound || 'No results found for'} "{ghQuery}"</p>
        )}
        {ghResults.length > 0 && (
              ...
              <div key={repo.id} className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5 hover:border-white/10 transition-all">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <a href={repo.url} target="_blank" rel="noopener noreferrer" className="text-white/20 hover:text-white/60">
                      <ExternalLink size={12} />
                    </a>
                  </div>
                  <p className="text-[10px] text-white/30 truncate mt-0.5">{repo.description || (t?.mcpNoDescription || 'No description')}</p>
                  <div className="flex items-center gap-3 mt-1">
                    <span className="text-[9px] text-white/20 flex items-center gap-1"><Star size={10} /> {repo.stars || 0}</span>
                    {repo.language && <span className="text-[9px] text-white/20">{repo.language}</span>}
                  </div>
                </div>
                <Button
                  onClick={() => handleInstallFromGitHub(repo.name)}
                  className="ml-3 bg-white/5 hover:bg-white/10 border border-white/10 text-[10px] font-black uppercase tracking-widest px-3 h-8 rounded-xl flex-shrink-0"
                >
