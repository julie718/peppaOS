...
describe('language detection', () => {
  it('treats Chinese messages with English product names as Chinese', () => {
    expect(detectLanguage('为什么我给peppa发消息会默认用英语反馈给我，我说的可是中文')).toBe('zh');
    expect(getResponseLanguage('为什么我给peppa发消息会默认用英语反馈给我，我说的可是中文')).toBe('Simplified Chinese');
  });

  it('treats Chinese messages with code terms as Chinese', () => {
    expect(detectLanguage('React useEffect 怎么用')).toBe('zh');
    expect(detectLanguage('peppa 帮我 fix bug')).toBe('zh');
  });
  ...
  it('builds an explicit anti-English-drift prompt overlay', () => {
    const instruction = buildResponseLanguageInstruction('peppa 帮我 fix bug');
