# Peppa Windows Deployment
# Run as Administrator for C:\Program Files install
#   ./scripts/deploy-windows.ps1
# Or for per-user install (no admin needed):
#   ./scripts/deploy-windows.ps1 -InstallDir "$env:LOCALAPPDATA\Peppa"

param(
  [string]$InstallDir = "$env:ProgramFiles\Peppa"
)
...
Write-Host "  Peppa Deployment" -ForegroundColor Cyan
...
# Copy exe
$exeSrc = "$ProjectDir\src-tauri\target\release\peppa-os.exe"
if (-not (Test-Path $exeSrc)) {
  Write-Host "ERROR: peppa-os.exe not found at $exeSrc" -ForegroundColor Red
  exit 1
}
Copy-Item $exeSrc "$InstallDir\peppa-os.exe" -Force
Write-Host "  peppa-os.exe" -ForegroundColor Gray
...
$shortcutPath = "$desktopPath\Peppa OS.lnk"
$WScriptShell = New-Object -ComObject WScript.Shell
$Shortcut = $WScriptShell.CreateShortcut($shortcutPath)
$Shortcut.TargetPath = "$InstallDir\peppa-os.exe"
$Shortcut.WorkingDirectory = $InstallDir
$Shortcut.Description = "Peppa OS - Personal AI"
if (Test-Path "$InstallDir\peppa-os.exe") {
  $Shortcut.IconLocation = "$InstallDir\peppa-os.exe,0"
}
$Shortcut.Save()
...
Write-Host "  Data: $env:USERPROFILE\Peppa\data\" -ForegroundColor Cyan
