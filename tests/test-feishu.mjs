...
async function main() {
    ...
    body: JSON.stringify({
      ...
      event: {
        ...
        message: {
          ...
          chat_type: 'group',
          message_type: 'text',
          content: JSON.stringify({ text: '你好 Peppa，今天天气怎么样？' }),
  ...
  const sendRes = await fetch(`${BASE}/feishu/send`, {
    ...
    body: JSON.stringify({
      ...
      text: '测试消息 — from Peppa test suite',
    }),
  });
  ...
  const cardRes = await fetch(`${BASE}/feishu/send`, {
    ...
    body: JSON.stringify({
      chatId: 'oc_testchat001',
      card: {
        title: 'Peppa Test Card',
        subtitle: 'Integration Test',
        body: 'This card was sent from the Peppa Feishu test suite.',
        color: 'blue',
